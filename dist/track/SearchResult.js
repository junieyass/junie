...
exports.buildSearchIdentifier = buildSearchIdentifier;
 ...
 */
function buildSearchIdentifier(query, source, defaultSource = 'youtube') {
    const trimmed = query.trim();
    if ((0, Helpers_js_1.isUrl)(trimmed) || (0, Helpers_js_1.hasSearchPrefix)(trimmed))
        return trimmed;
    const activeSource = source ?? defaultSource;
    if (activeSource === 'none')
        return trimmed;
    const prefix = constants_js_1.SOURCE_PREFIXES[activeSource];
    return `${prefix ?? `${activeSource}:`}${trimmed}`;
}
/** Turn a raw `/v4/loadtracks` response into a {@link SearchResult}. */
function buildSearchResult(response, node, requester) {
    const tracks = [];
    let playlist = null;
    let exception = null;
    switch (response.loadType) {
        case 'track': {
            ...
            if (data && typeof data === 'object' && data.encoded) {
                tracks.push(new Track_js_1.Track(data, requester));
            }
            break;
        }
        case 'playlist': {
            ...
            for (const apiTrack of data)
                tracks.push(new Track_js_1.Track(apiTrack, requester));
