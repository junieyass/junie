...
exports.VOICE_ZONES = void 0;
exports.parseVoiceRegion = parseVoiceRegion;
exports.regionZone = regionZone;
exports.regionPenalty = regionPenalty;
 ...
 */
function parseVoiceRegion(endpoint) {
    if (!endpoint)
        return null;
 ...
 */
function regionPenalty(nodeRegions, endpoint) {
    if (!nodeRegions || nodeRegions.length === 0)
        return 0;
    const voiceZone = regionZone(parseVoiceRegion(endpoint));
    if (voiceZone === 'unknown')
        return 250;
    for (const configured of nodeRegions) {
        if (regionZone(configured) === voiceZone)
            return 0;
    }
    return 1000;
}
