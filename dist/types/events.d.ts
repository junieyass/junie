...
import type { LavalinkException, NodeStats, PlayerState, TrackEndReason, WebSocketPayload, VoiceState } from './api.js';
...
export interface JunieEvents<TRequester = unknown> {
    ...
    nodeDisconnect: (node: Node, info: {
        code: number;
        reason: string;
    }) => void;
    /** A node's next reconnect attempt has been scheduled. */
    nodeReconnecting: (node: Node, info: {
        attempt: number;
        delay: number;
    }) => void;
    ...
    playerVoiceClosed: (player: Player<TRequester>, info: {
        code: number;
        reason: string;
        byRemote: boolean;
    }) => void;
...
export interface NodeEvents {
    ...
    disconnect: (node: Node, info: {
        code: number;
        reason: string;
    }) => void;
    reconnecting: (node: Node, info: {
        attempt: number;
        delay: number;
    }) => void;
