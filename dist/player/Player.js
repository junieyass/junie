...
exports.Player = exports.defaultAutoplayResolver = void 0;
const constants_js_1 = require("../constants.js");
const errors_js_1 = require("../errors.js");
const Queue_js_1 = require("../queue/Queue.js");
const Track_js_1 = require("../track/Track.js");
const TypedEmitter_js_1 = require("../utils/TypedEmitter.js");
const Helpers_js_1 = require("../utils/Helpers.js");
const FilterManager_js_1 = require("./FilterManager.js");
/** The default autoplay resolver: YouTube search seeded by the last track. */
const defaultAutoplayResolver = async (_player, lastTrack) => {
    const query = lastTrack.isStream
        ? lastTrack.title
        : `${lastTrack.title} ${lastTrack.author}`;
    return { query, source: 'youtube' };
};
exports.defaultAutoplayResolver = defaultAutoplayResolver;
/**
 * A guild's audio player.
 */
class Player extends TypedEmitter_js_1.TypedEmitter {
    /** The owning client. */
    junie;
    ...
    constructor(junie, node, options) {
        ...
        this.volume = options.volume ?? constants_js_1.DEFAULTS.player.volume;
        this.autoplay = options.autoplay ?? false;
        this.queue = new Queue_js_1.Queue(this, {
        ...
        this.filters = new FilterManager_js_1.FilterManager(this);
    }
    // -------------------------------------------------------------------------
    // Small conveniences
    // -------------------------------------------------------------------------
    ...
    assertAlive() {
        if (this.lifecycle === 'destroyed' || this._destroying) {
            throw new errors_js_1.JunieError(errors_js_1.JunieErrorCode.PLAYER_DESTROYED, `Player for guild ${this.guildId} is destroyed.`, { guildId: this.guildId });
        }
    }
    // -------------------------------------------------------------------------
    // Voice
    ...
    async connect() {
        ...
        if (!this.voiceChannelId) {
            throw new errors_js_1.JunieError(errors_js_1.JunieErrorCode.INVALID_ARGUMENT, 'Cannot connect without a voice channel id.', { guildId: this.guildId });
        }
        ...
        const timeoutMs = this.junie.options.voiceConnectionTimeout ?? constants_js_1.DEFAULTS.voiceConnectionTimeout;
        let timer;
        const waiter = new Promise((resolve, reject) => {
            ...
            timer = setTimeout(() => {
                ...
                reject(new errors_js_1.VoiceConnectionError(this.guildId, timeoutMs));
            }, timeoutMs);
        });
    ...
    async sendGatewayPayload(channelId) {
        const payload = {
            op: constants_js_1.GATEWAY_VOICE_STATE_OPCODE,
            d: {
                guild_id: this.guildId,
                channel_id: channelId,
                self_mute: this.junie.options.player.selfMute ?? constants_js_1.DEFAULTS.player.selfMute,
                self_deaf: this.junie.options.player.selfDeaf ?? constants_js_1.DEFAULTS.player.selfDeaf,
            },
        };
    ...
    async play(track, options = {}) {
        ...
        const input = typeof track === 'string'
            ? new Track_js_1.UnresolvedTrack(track)
            : track;
    ...
    async seek(position) {
        ...
        if (!current.isSeekable) {
            throw new errors_js_1.JunieError(errors_js_1.JunieErrorCode.TRACK_NOT_SEEKABLE, `Cannot seek the current track ("${current.title}") — it is a live stream.`, { guildId: this.guildId, title: current.title });
        }
        const clamped = (0, Helpers_js_1.clamp)(Math.round(position), 0, current.length);
    ...
    async setVolume(volume) {
        this.assertAlive();
        const clamped = (0, Helpers_js_1.clamp)(Math.round(volume), constants_js_1.PLAYER_VOLUME_RANGE.min, constants_js_1.PLAYER_VOLUME_RANGE.max);
    ...
    async setNode(target) {
        ...
        if (!newNode.connected) {
            throw new errors_js_1.JunieError(errors_js_1.JunieErrorCode.NODE_CONNECTION_FAILED, `Target node "${newNode.id}" is not connected.`, { guildId: this.guildId, node: newNode.id });
        }
        ...
        if (voice)
            payload.voice = voice;
        if (this.queue.current instanceof Track_js_1.Track) {
        ...
        try {
            await (0, Helpers_js_1.withTimeout)(oldNode.rest.destroyPlayer(this.guildId), constants_js_1.DESTROY_TIMEOUT);
        }
        catch {
    ...
    async destroy(reason = 'manual') {
        ...
        try {
            await (0, Helpers_js_1.withTimeout)(this.node.rest.destroyPlayer(this.guildId), constants_js_1.DESTROY_TIMEOUT);
        }
        catch (error) {
    ...
    async _startTrack(target, options = {}) {
        ...
        try {
            ...
            if (error instanceof errors_js_1.TrackLoadError) {
                ...
                if (this.junie.options.skipOnError ?? constants_js_1.DEFAULTS.skipOnError) {
    ...
    async _resolveInput(target) {
        if (target instanceof Track_js_1.Track)
            return target;
    ...
    async _tryAutoplay() {
        ...
        if (!last || !(last instanceof Track_js_1.Track)) {
            this.emitPublic('queueEnd', this);
            this.junie.forwardPlayerEvent(this, 'queueEnd');
            return;
        }
        try {
            const resolver = (this.junie.options.autoplayResolver ?? exports.defaultAutoplayResolver);
            ...
            if (Array.isArray(output)) {
                ...
                const seen = new Set(this.queue.previous
                    .slice(-10)
                    .filter((entry) => entry instanceof Track_js_1.Track)
    ...
    _handleTrackStart(event) {
        ...
        if (this.queue.current instanceof Track_js_1.Track &&
            ...
            this.queue.current = new Track_js_1.Track(event.track, this.queue.current?.requester);
        }
        this.playing = true;
        this.paused = false;
    ...
    _handleTrackEnd(event) {
        ...
        if (event.reason === 'loadFailed' && !(this.junie.options.skipOnError ?? constants_js_1.DEFAULTS.skipOnError)) {
            this.playing = false;
            return;
        }
    ...
    _handleVoiceClosed(event) {
        ...
        const autoReconnect = this.junie.options.autoVoiceReconnect ?? constants_js_1.DEFAULTS.autoVoiceReconnect;
        if (!autoReconnect ||
            !event.byRemote ||
            !this.voiceChannelId ||
            this.lifecycle === 'destroyed' ||
            this._destroying ||
            this._voiceReconnects >= constants_js_1.MAX_VOICE_RECONNECT_ATTEMPTS) {
            return;
        }
        ...
        this.junie.logger.info(`Voice socket closed (code ${event.code}) for guild ${this.guildId} — rejoining in ${delay}ms ` +
            `(attempt ${this._voiceReconnects}/${constants_js_1.MAX_VOICE_RECONNECT_ATTEMPTS}).`);
        setTimeout(() => {
    ...
    _matchTrack(raw) {
        if (this.queue.current instanceof Track_js_1.Track && this.queue.current.encoded === raw.encoded) {
            return this.queue.current;
        }
        return new Track_js_1.Track(raw, this.queue.current?.requester);
    }
    // -------------------------------------------------------------------------
