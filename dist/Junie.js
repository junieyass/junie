...
exports.Junie = void 0;
const constants_js_1 = require("./constants.js");
const errors_js_1 = require("./errors.js");
const NodeManager_js_1 = require("./node/NodeManager.js");
const PenaltyStrategy_js_1 = require("./node/strategies/PenaltyStrategy.js");
const PlayerManager_js_1 = require("./player/PlayerManager.js");
...
function resolveOptions(raw) {
    return {
        ...raw,
        userId: raw.userId ?? '',
        clientName: raw.clientName ?? constants_js_1.DEFAULT_CLIENT_NAME,
        strategy: raw.strategy ?? new PenaltyStrategy_js_1.PenaltyStrategy(),
        defaultSearchSource: raw.defaultSearchSource ?? constants_js_1.DEFAULTS.defaultSearchSource,
        searchParallel: raw.searchParallel ?? constants_js_1.DEFAULTS.searchParallel,
        skipOnError: raw.skipOnError ?? constants_js_1.DEFAULTS.skipOnError,
        autoVoiceReconnect: raw.autoVoiceReconnect ?? constants_js_1.DEFAULTS.autoVoiceReconnect,
        destroyOnVoiceLeave: raw.destroyOnVoiceLeave ?? true,
        autoFailover: raw.autoFailover ?? constants_js_1.DEFAULTS.autoFailover,
        voiceConnectionTimeout: raw.voiceConnectionTimeout ?? constants_js_1.DEFAULTS.voiceConnectionTimeout,
        autoplayResolver: raw.autoplayResolver,
        reconnect: { ...constants_js_1.DEFAULTS.reconnect, ...raw.reconnect },
        resume: { ...constants_js_1.DEFAULTS.resume, ...raw.resume },
        rest: { timeout: constants_js_1.DEFAULTS.rest.timeout, retries: constants_js_1.DEFAULTS.rest.retries, headers: {}, ...raw.rest },
        queue: {
            store: raw.queue?.store,
            restore: raw.queue?.restore ?? constants_js_1.DEFAULTS.queue.restore,
            historyLimit: raw.queue?.historyLimit ?? constants_js_1.DEFAULTS.queue.historyLimit,
        },
        webSocketFactory: raw.webSocketFactory,
        logLevel: raw.logLevel ?? constants_js_1.DEFAULTS.logLevel,
        logger: raw.logger,
        player: { ...constants_js_1.DEFAULTS.player, ...raw.player },
    };
}
/**
 ...
 */
class Junie extends TypedEmitter_js_1.TypedEmitter {
    ...
    utils = {
        ...
        buildTrack: (data, requester) => new Track_js_1.Track(data, requester),
        /** `mm:ss` / `hh:mm:ss` formatting (streams render as "LIVE"). */
        formatDuration: Helpers_js_1.formatDuration,
        /** Extract the Discord voice region token from an endpoint. */
        parseVoiceRegion: Regions_js_1.parseVoiceRegion,
        /** Map a region token to a coarse zone. */
        regionZone: Regions_js_1.regionZone,
    };
    destroyed = false;
    constructor(options) {
        super();
        ...
        this.logger = this.options.logger ?? (0, Logger_js_1.createDefaultLogger)(this.options.logLevel);
        this.nodes = new NodeManager_js_1.NodeManager(this, this.options.strategy, {
            reconnect: this.options.reconnect,
            resume: this.options.resume,
            rest: this.options.rest,
        });
        this.players = new PlayerManager_js_1.PlayerManager(this);
    ...
    init(userId) {
        if (userId)
            this.userId = userId;
        if (!this.userId) {
            throw new errors_js_1.JunieError(errors_js_1.JunieErrorCode.MISSING_USER_ID, 'No user id — pass userId in the Junie options or call Junie#init(userId) after login.');
        }
