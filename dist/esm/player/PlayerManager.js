...
export class PlayerManager {
    ...
    create(options) {
        ...
        const player = new Player(this.junie, node, options);
    ...
    require(guildId) {
        ...
        if (!player) {
            throw new JunieError(JunieErrorCode.PLAYER_NOT_FOUND, `No player for guild ${guildId}.`, { guildId });
        }
        return player;
    }
    ...
    resolveNode(options) {
        if (options.node) {
            ...
            if (!pinned) {
                throw new JunieError(JunieErrorCode.NODE_NOT_FOUND, `Cannot create a player on unknown node "${options.node}".`, { node: options.node });
            }
            if (!pinned.connected) {
