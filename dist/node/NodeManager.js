...
class NodeManager {
    ...
    create(option) {
        if (this.nodes.has(option.id)) {
            throw new errors_js_1.JunieError(errors_js_1.JunieErrorCode.NODE_ALREADY_EXISTS, `A node with id "${option.id}" already exists.`, { id: option.id });
        }
        const node = new Node_js_1.Node(this.host, option, this.clientDefaults);
    ...
    require(id) {
        ...
        if (!node) {
            throw new errors_js_1.JunieError(errors_js_1.JunieErrorCode.NODE_NOT_FOUND, `No node with id "${id}".`, { id });
        }
        return node;
    }
    ...
    async fanOutSearch(query, requester) {
        ...
        if (nodes.length === 0) {
            throw new errors_js_1.JunieError(errors_js_1.JunieErrorCode.NO_HEALTHY_NODES, 'No connected Lavalink node available for the search.');
        }
        if (nodes.length === 1) {
            ...
            const finish = () => {
                ...
                if (lastEmpty) {
                    ...
                    reject(firstError instanceof Error
                        ? firstError
                        : new errors_js_1.TrackLoadError('All nodes failed or returned no results.', { query: queryObject.query }));
                }
            };
