...
exports.normalizeQueueInput = normalizeQueueInput;
...
function normalizeQueueInput(input) {
    ...
    for (const item of items) {
        const revived = (0, Track_js_1.reviveTrackLike)(item);
        if (!revived) {
            throw new errors_js_1.JunieError(errors_js_1.JunieErrorCode.INVALID_ARGUMENT, 'Queue input must be a Track, UnresolvedTrack, or raw Lavalink track object.', { received: typeof item });
        }
...
class Queue {
    ...
    setRepeatMode(mode) {
        if (mode !== 'off' && mode !== 'track' && mode !== 'queue') {
            throw new errors_js_1.JunieError(errors_js_1.JunieErrorCode.INVALID_ARGUMENT, `Invalid repeat mode "${String(mode)}" — expected 'off' | 'track' | 'queue'.`);
        }
    ...
    add(input, position) {
        ...
        if (position === undefined) {
            ...
            const index = (0, Helpers_js_1.clamp)(position < 0 ? this.tracks.length + position : position, 0, this.tracks.length);
     ...
     */
    shuffle(seed) {
        (0, Helpers_js_1.shuffleInPlace)(this.tracks, (0, Helpers_js_1.createRng)(seed ?? Date.now()));
        this.persist();
        return this;
    }
    /**
    ...
    move(from, to) {
        ...
        if (!moved)
            return null;
        const target = (0, Helpers_js_1.clamp)(to < 0 ? this.tracks.length + to : to, 0, this.tracks.length);
    ...
    toJSON() {
        const serialize = (track) => track instanceof Track_js_1.Track || track instanceof Track_js_1.UnresolvedTrack ? track.toJSON() : track;
        return {
    ...
    fromJSON(data) {
        const revive = (value) => (0, Track_js_1.reviveTrackLike)(value);
