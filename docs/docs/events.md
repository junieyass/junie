...
> **Node-level extra:** each `Node` also emits `versionMismatch`
> `(node, { version, expected })` when the server reports a Lavalink version
> whose major differs from the v4 protocol Junie implements — a
> protocol-drift early warning. The connection is never broken. Listen with
> `node.on('versionMismatch', …)`; the detected version is available as
> `node.lavalinkVersion`.
