...
An *unplanned* node death — crash, network partition, host reboot — is handled
for you. When a node's WebSocket closes and other nodes are connected, the
client immediately migrates every player of the dead node onto the best
remaining node (same selection strategy, region-aware). The migration is the
same live-migration `setNode` performs: voice, current track, position,
volume and filters are re-established on the target in one REST round trip,
then the old player is destroyed. Playback never notices beyond a transport blip.

```ts
new Junie({
  // …
  autoFailover: true, // default; set false to handle node death yourself
});
```

When *no* other node is connected, migration is skipped and the players wait
for their node's reconnect — a fresh session rebuilds them remotely
(404 self-healing), a resumed session continues them where they were.

## Version radar

After every `ready`, each node asks the server for its version
(`GET /version`) and stores it on `node.lavalinkVersion`. When the server's
major version differs from the v4 protocol Junie implements, the node emits
`versionMismatch` and logs a warning — the connection is never broken. This
is the early-warning radar for protocol drift: when Lavalink ships a new
major, your logs tell you before your users do.

```ts
node.on('versionMismatch', (node, { version, expected }) => {
  ops.alert(`Node ${node.id} runs Lavalink ${version}, Junie targets v${expected}`);
});
```
